...
// top of the real canvas. Mirrors the committed CurveField, kept current via
// resync() whenever the composition's history changes (new op, undo, redo,
// reset, backend switch) -- not on every preview frame, so the O(history)
// cost of a full replay is paid only on commit, the same way the renderers'
// own undo/redo replay already does. Each preview frame just clones the
// already-current mirror and applies the one candidate operation to the
// clone, so its cost depends on the current drop/point count, not on how
// many operations led up to it.
//
// Only viable for the vector renderer backends (worker or main-thread): the
// GPU renderer has no equivalent committed-state mirror to preview against,
// so this is hidden via setEnabled() while it's active.
